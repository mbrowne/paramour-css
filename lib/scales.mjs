 ...
 * @param {Object} config.multipliers - Custom multipliers for the space scale. If specified, these override scaleMin and scaleMax
...
export function generateScaleProperties({
  ...
  const clampSteps = stepsArray.reduce((clampSteps, step) => {
    ...
    return [
      ...
      {
        [step]: generateClamp({
          baseMinPx: roundToHundreths(baseMin * (multiplier ?? Math.pow(scaleMinValue, step) )),
          baseMaxPx: roundToHundreths(baseMax * (multiplier ?? Math.pow(scaleMaxValue, step) )),
